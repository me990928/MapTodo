...
class LargeDataViewModel: ObservableObject {
    ...
    func update(data: MapDataModel, modelContext: ModelContext,complete: @escaping (Bool)->Void){
        ...
        let newItem = MapDataModel(id: data.id, title: model.title, subTitle: model.subTitle, lat: data.lat, lon: data.lon, registDate: data.registDate, endDate: data.endDate, endFlag: model.todoFlag, mapMode: data.mapMode, mapMemo: model.memoData, addLater: data.addLater)
...
//                    print("success \(newItem.lat)")
    ...
    func endFlagAct(data:MapDataModel, flag:Bool, modelContext: ModelContext, complete: @escaping (Bool)->Void){
        ...
        let newItem = MapDataModel(id: data.id, title: data.title, subTitle: data.subTitle, lat: data.lat, lon: data.lon, registDate: data.registDate, endDate: Date(), endFlag: flag, mapMode: data.mapMode, mapMemo: data.mapMemo, addLater: data.addLater)
