...
struct MapCard: View {
    ...
    func update(){
        let updateModel = MapDataModel(id: data.id, title: data.title, subTitle: data.subTitle, lat: data.lat, lon: data.lon, registDate: data.registDate, endDate: data.endDate, endFlag: data.endFlag, mapMode: mapStyler == .real ? true : false, mapMemo: data.mapMemo)
